...
python -u run_longExp.py \
  ...
  --root_path ./dataset/ \
  --data_path weather.csv \
  --model_id weather_$seq_len'_'384 \
  --model $model_name \
  --data custom \
  --features S \
  --seq_len $seq_len \
  ...
  --enc_in 21 \
  --des 'Exp' \
  --itr 1 --batch_size 16  >logs/LongForecasting/$model_name'_'Weather_$seq_len'_'384.log

python -u run_longExp.py \
  --is_training 1 \
  --root_path ./dataset/ \
  --data_path weather.csv \
  --model_id weather_$seq_len'_'768 \
  --model $model_name \
  --data custom \
  --features S \
  --seq_len $seq_len \
  ...
  --enc_in 21 \
  --des 'Exp' \
  --itr 1 --batch_size 16  >logs/LongForecasting/$model_name'_'Weather_$seq_len'_'768.log
done